...
class Appwindow(tk.Tk):
    ...
    def jtsb1(self):
        ...
        while self.stop != 1 and n < limit:
            ...
            time.sleep(0.25)
            # print("刷新点击")
            # print(self.savedata["repeatbtn"])
            # print(f"life:{self.lifewindow}")
        ...
        if i > 0:
            ...
            if self.lifewindow.title == "火影忍者online":
                ...
                try:
                    ...
                    # 计算缩放比例
                    scale_x = self.screen_width / 3072
                    scale_y = self.screen_height / 1920

                    self.lifewindow.top = round(38 * scale_y)
                    self.lifewindow.left = round(160 * scale_x)
                    self.lifewindow.width = round(2804 * scale_x)
                    self.lifewindow.height = round(1740 * scale_y)

                    # print(f"窗口位置: 左{self.lifewindow.left} 上{self.lifewindow.top}")
                    # print(f"窗口大小: {self.lifewindow.width}x{self.lifewindow.height}")
    ...
    def queuespeed(self):
        ...
        if not self.youxianjibiao:
            ...
            self.sonwindow1.bind("<Control-s>",self.yxjused)
    ...
    def yxjused(self,event=None): # event接受键盘传入
        self.youxianjibiao = {}
    ...
    def modchoose(self):
        ...
        self.sonwindow2.bind("<Control-s>", self.setused)
        ...
        self.sbtn20 = tk.Button(self.sonwindow2, text="保存设置(s)", width=10, fg="red", bg="#FFFFFF", command=self.setused)
    ...
    def setused(self,event=None): #绑定按键触发时会有名为event参数传入，所以要定义接受
        # print("精度修改使用")
